...
function usecases({ maindiv = 'bg-white', img='rounded-full', image, name, subcontent, content, showbutton=true, link = '' }) {
    return (
        <>
            <div className={` px-3 md:px-15 py-3 md:py-7 rounded-lg flex items-center md:items-end gap-3 md:gap-30 ${maindiv} `}>
                ...
                <div className='text-left '>
                    ...
                    {showbutton && (
                        <Link to={link} >
                            <button className='cursor-pointer w-[80px] md:w-[170px] h-[20px] md:h-[45px] text-[#FD6B70] hover:text-white hover:bg-[#FD6B70] border border-[#FD6B70] rounded-4xl text-[9px] md:text-[20px] '>Get Started</button>
                        </Link>
                    )}
                </div>
            </div>
