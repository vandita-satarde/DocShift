...
function StoragePlan({color='', subtext, name, description, showButton=true }) {
    return (
        <>
            <div className={`tracking-wider md:tracking-widest text-center pt-7 md:pt-14 pb-7 md:pb-15 text-[#0E1836] ${color} `}>
                <p className='text-[9px] md:text-[24px] font-semibold md:-mb-2'>{subtext}</p>
                <p className='text-[18px] md:text-[48px] '>{name}</p>
                <p className='text-[8px] md:text-[13px] md:py-4 leading-3 md:leading-7 mx-8 '>{description}</p>
                {showButton && (
